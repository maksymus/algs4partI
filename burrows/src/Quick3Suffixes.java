public class Quick3Suffixes {
    ...
    public static void sort(String a, Suffix[] inds) {
        sort(a, inds, 0, inds.length-1, 0);
    }

    // return the dth character of suffix starting at s[sufIdx],
    // -1 if d = length of s
    private static int charAt(String s, Suffix sufIdx, int d) {
        if (d >= s.length())
            return -1;
        return sufIdx.charAt(d);
    }


    // 3-way string quicksort inds[lo..hi] starting at dth character
    private static void sort(String a, Suffix[] inds, int lo, int hi, int d) {
    ...
    private static void insertion(String a, Suffix[] inds, int lo, int hi, int d) {
        for (int i = lo; i <= hi; i++)
    ...
    private static boolean less(String a, Suffix sufInd1, Suffix sufInd2, int d) {
